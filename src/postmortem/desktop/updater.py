...
# failed second move (a different volume, a permissions change, an
# antivirus hold) exited with nothing at the install path and nothing
# relaunched. Now the move aside is retried, a failed install restores the
# backup and relaunches the OLD bundle, and something is always launched
# at the end.
...
NEW_APP="$3"
BACKUP_APP="$4"
LOG="${TMPDIR:-/tmp}/postmortem-desktop-update.log"
...
# Step 1: old bundle -> backup, retried while the OS lets go of it.
moved_aside=0
if [ -e "$OLD_APP" ]; then
  i=0
  while [ "$i" -lt 20 ]; do
    if mv "$OLD_APP" "$BACKUP_APP" 2>>"$LOG"; then
      moved_aside=1
      break
    fi
    i=$((i + 1))
  ...
  if [ "$moved_aside" -ne 1 ]; then
    log "could not move the current app aside; relaunching it unchanged"
    open -n "$OLD_APP" || true
    exit 0
  fi
fi
...
# back and relaunch that, so there is always an app at $OLD_APP.
if ! mv "$NEW_APP" "$OLD_APP" 2>>"$LOG"; then
  log "could not install the new app; restoring the previous one"
  if [ "$moved_aside" -eq 1 ]; then
    mv "$BACKUP_APP" "$OLD_APP" 2>>"$LOG" || log "restore failed; the backup is at $BACKUP_APP"
  fi
  open -n "$OLD_APP" || true
  exit 0
fi
